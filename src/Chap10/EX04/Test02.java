...
class Animal3{ 			// 부모 클래스, 모든 클래스는 Object 클래스를 상속한다
	...
	@Override 		// Object.toString 메소드는 객체 자체를 출력할 때 호출이 된다.
	public String toString() {				// 객체의 주소를 출력합니다. [ 패키지명.클래스명@객체의 메모리의 해쉬코드 ]
		return "이름은 : " + name + " , 나이는" + age + " 입니다. "; 		// 객체 주소 출력하는 toString을 @Override로 재정의 한 거임. 
																	// 원래 Chap10.EX04.Animal3@32d2fa64
	}
}
...
class Tiger3 extends Animal3{
	...
	Tiger3(){}; 	// 기본 생성자 : 변수의 초기화 값을 할당 하지 않고 초기화.
					// 다른 생성자가 객체 내에 존재할 경우 기본생성자를 외부에서 호출시 생략되어 있으면 안된다. 
	
	Tiger3(String a, int b){		// 생성자 : 객체를 생성할 떄 메모리의 초기값을 할당할 때 사용.
	...
	@Override
	void cry() {
		super.cry(); 		// super 키워드는 부모클래스, super : 상속 관계일 때 부모 클래스
							// this 키워드는 자기자신의 객체
...
public class Test02 {
	...
	public static void main(String[] args) {
		...
		System.out.println(a3); 		// 객체 자체를 출력할 경우, Object 클래스의 toString 메소드가 호출
						// Object.toString() : 객체의 메모리주소의 해쉬코드를 출력
						// Chap10.EX04.Animal3@32d2fa64 <== 패키지명.객체명@해쉬코드
		
		System.out.println(a3.toString());
		
		// 객체 자체를 출력하면 Object 클래스의 toSting 메소드를 호출
		...
		c3.cry(); 		// 부모의 cry()를 호출 할 경우 오버라이딩된 Cat3의 cry()를 호출
		d3.cry();
		...
		System.out.println(aa3); 	// 객체 자체를 출력시, 기본적으로 객체의 메모리 해쉬코드가 출력
		System.out.println(cc3);	// Object.toString() 메소드를 Animal3 class에서 재정의 함.
