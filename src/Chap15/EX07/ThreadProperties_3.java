...
// 데몬 쓰레드 : setDaemon(true), 쓰레드를 start 할 때 설정, 수정 변경은 불가
// - 마지막 쓰레드가 종료되는 시점에 종료
// - 일반적으로 다른 쓰레드의 작업을 돕는 보조 쓰레드
// - 종료시점이 마지막 일반쓰레드 종료될  때 종료점 외에는 일반쓰레드와 동일
// - 가비지 컬렉트(메모리의 사용되지 않는 객체를 가비지 컬렉터가 자동으로 객체를 제거)
// 		- 프로세서가 종료시점에 자동저장, 화면 자동 갱신 ...
//	- 무한 루프를 사용해서 항시 대기하고 있다가 특정 조건이 만족되면 
...
public class ThreadProperties_3 {
	...
	public static void main(String[] args) {
		...
		// 2. Daemon 쓰레드 : 마지막 쓰레드가 종료 되는 시점에 종료
