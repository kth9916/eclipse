...
public class ReadArrayData_2D_Ex09 {
	public static void main(String[] args) {
		...
		for ( int i = 0; i < array2.length; i++) {			//바같쪽 for 문 : 행을 루프 돌린다
			for( int j = 0; j < array2[i].length; j++) {	//안쪽 for 문 : 열을 루프 돌린다
		...
		for( int[] array : array2) {		//2차원 배열의 한 행의 값을 1차원 배열 변수에 담고  (array)
			for( int k : array) {			//배열의 값을 변수에 담아서 출력, k/array는 임의로 명명한 것
				System.out.print(k + " ");
			}
			System.out.println();
		}
