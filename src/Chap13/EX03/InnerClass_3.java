...
// static 이너 클래스 : 외부 클래스의 인스턴스 필드, 인스턴스 메소드는 접근이 불가
					// 외부클래스의 static 필드나 static 메소드는 접근이 가능

class A {
	int a = 3; 		// 인스턴스 필드, 객체화 해야 접근 가능
	static int b = 4 ;	// static 필드, 객체 생성 없이 접근, 객체 생성 후 접근
	void method1() { 	// 인스턴스 메소드 : 객체 생성 후 접근
		System.out.println("instance Method");
	}
	static void method2() { 		// static 메소드, 객체를 생성하지 않고 접근, 객체 성성 후 접근
		System.out.println("static Method");
	}
	static class B{ // static 이너클래스 : 외부 객체를 생성하지 않고 객체 생성이 가능
		void bcd() {
