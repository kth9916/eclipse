...
public class FileMethod {
	...
	public static void main(String[] args) {
		...
		System.out.println(newFile1.mkdir()); 		// 폴더가 존재하는 경우 : true , 폴더가 존재해서 생성되지 못한 경우 false
			// 여러이유로 인해서 false : 생성을 못했다. (해당폴더가 존재하는 경우, 권한 문제로 만들 수 없는 경우)
		System.out.println(newFile1.exists());
		
		File newFile2 = new File("C:/temp/bcd/cde"); 	// 하위의 폴더를 한번에 생성
		System.out.println(newFile2.mkdir()); 			// false, 하위 폴더까지 한꺼번에 생성이 안된다
		System.out.println(newFile2.mkdirs());			// true, 하위 폴더까지 한꺼번에 생성 가능
		...
		for(File k : fname) {
			System.out.println((k.isDirectory() ? "폴더 : " : "파일 : ")+ k.getName()); 	// 삼항 연산자  --> if ? true : false
		}
